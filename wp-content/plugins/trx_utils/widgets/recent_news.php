...
// Theme init
if (!function_exists('soapery_widget_recent_news_theme_setup')) {
	add_action( 'soapery_action_before_init_theme', 'soapery_widget_recent_news_theme_setup', 1 );
	function soapery_widget_recent_news_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('soapery_action_shortcodes_list',	'soapery_widget_recent_news_reg_shortcodes');
		if (function_exists('soapery_exists_visual_composer') && soapery_exists_visual_composer())
			add_action('soapery_action_shortcodes_list_vc','soapery_widget_recent_news_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('soapery_widget_recent_news_load')) {
	add_action( 'widgets_init', 'soapery_widget_recent_news_load' );
	function soapery_widget_recent_news_load() {
		register_widget('soapery_widget_recent_news');
	}
}


// Widget Class
class soapery_widget_recent_news extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_news', 'description' => esc_html__('Show recent news in many styles', 'soapery'));
		parent::__construct( 'soapery_widget_recent_news', esc_html__('Soapery - Recent News', 'soapery'), $widget_ops );
	}
	...
	function widget($args, $instance) {
		...
		$output = soapery_sc_recent_news( array(
			'from_widget'		=> true,
		...
		if (!empty($output)) {
			...
			if ($widget_title) soapery_show_layout($title, $before_title, $after_title);
	...
	function form($instance) {
		...
		$posts_types = soapery_get_list_posts_types(false);
		$categories = soapery_get_list_terms(false, soapery_get_taxonomy_categories_by_post_type($post_type));
		$news_styles = soapery_get_list_templates('news');
		?>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('widget_title')); ?>"><?php esc_html_e('Widget title:', 'soapery'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Block title:', 'soapery'); ?></label>
			<input id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr($title); ?>" class="widgets_param_fullwidth" />
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('subtitle')); ?>"><?php esc_html_e('Block subtitle:', 'soapery'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('style')); ?>"><?php esc_html_e('Style:', 'soapery'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('post_type')); ?>"><?php esc_html_e('Post type:', 'soapery'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('category')); ?>"><?php esc_html_e('Category:', 'soapery'); ?></label>
			<select id="<?php echo esc_attr($this->get_field_id('category')); ?>" name="<?php echo esc_attr($this->get_field_name('category')); ?>" class="widgets_param_fullwidth">
				<option value="0"><?php esc_html_e('-- Any category --', 'soapery'); ?></option> 
			<?php
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php esc_html_e('Number of posts to be displayed:', 'soapery'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('featured')); ?>"><?php esc_html_e('Number of featured posts:', 'soapery'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('columns')); ?>"><?php esc_html_e('Number of columns:', 'soapery'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_1"><?php esc_html_e('Show categories:', 'soapery'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_1"><?php esc_html_e('Show', 'soapery'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_0"><?php esc_html_e('Hide', 'soapery'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('Show counters:', 'soapery'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('Show', 'soapery'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_0"><?php esc_html_e('Hide', 'soapery'); ?></label>
		</p>
...
*/
if ( !function_exists( 'soapery_sc_recent_news' ) ) {
	function soapery_sc_recent_news($atts, $content=null){	
		...
		// Get categories list
		if (soapery_param_is_on($show_categories)) {
			if ( !soapery_storage_isset('categories_'.$category) ) {
				soapery_storage_set('categories_'.$category, get_categories(array(
					'orderby' => 'name',
					'parent' => $category
					))
				);
		...
		if (empty($from_widget)) {
			$widget_args = soapery_prepare_widgets_args(soapery_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_news', 'widget_recent_news');
			$output .= '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_recent_news_wrap' 
								. (soapery_exists_visual_composer() ? ' vc_recent_news wpb_content_element' : '') 
		...
		// Header
		if ( !empty($title) || !empty($subtitle) || (soapery_param_is_on($show_categories) && !soapery_storage_empty('categories_'.$category)) ) {
			$output	.= '<div class="sc_recent_news_header'.(soapery_param_is_on($show_categories) && !soapery_storage_empty('categories_'.$category) ? ' sc_recent_news_header_split' : '').'">'
			...
			// Categories list
			$cat_list = soapery_storage_get('categories_'.$category);
			if (soapery_param_is_on($show_categories) && !empty($cat_list)) {
				...
				if (is_array($cat_list) && count($cat_list) > 0) {
					$output .= '<a href="' . esc_url( $category == 0 
						...
						: get_category_link($category) ) . '" class="sc_recent_news_header_category_item">'.esc_html__('All News', 'soapery').'</span>';
					$number = 0;
					$number_max = 3;
					foreach ($cat_list as $cat) {
						$number++;
						if ($number == $number_max)
							$output .= '<span class="sc_recent_news_header_category_item sc_recent_news_header_category_item_more">'.esc_html__('More', 'soapery')
		...
		$args = soapery_query_add_posts_and_cats($args, $ids, $post_type, $category, $taxonomy);
		...
		return apply_filters('soapery_shortcode_output', $output, 'trx_recent_news', $atts, $content);
	}
	soapery_require_shortcode("trx_recent_news", "soapery_sc_recent_news");
}


// Add [trx_recent_news] in the VC shortcodes list
if (!function_exists('soapery_widget_recent_news_reg_shortcodes_vc')) {
	//add_action('soapery_action_shortcodes_list_vc','soapery_widget_recent_news_reg_shortcodes_vc');
	function soapery_widget_recent_news_reg_shortcodes_vc() {
		
		$posts_types = soapery_get_list_posts_types(false);
		$categories = soapery_get_list_terms(false, soapery_get_taxonomy_categories_by_post_type('post'));
		$news_styles = soapery_get_list_templates('news');

		vc_map( array(
				"base" => "trx_recent_news",
				"name" => esc_html__("Recent News", 'soapery'),
				"description" => wp_kses_data( __("Insert recent news list", 'soapery') ),
				...
				"params" => array(
					array(
						"param_name" => "style",
						"heading" => esc_html__("List style", 'soapery'),
						"description" => wp_kses_data( __("Select style to display news list", 'soapery') ),
						"class" => "",
						"admin_label" => true,
						"std" => 'news-magazine',
					...
					array(
						...
						"description" => wp_kses_data( __("Title for the widget (fill this field only if you want to use shortcode as widget)", 'soapery') ),
						"admin_label" => true,
						"class" => "",
						"value" => "",
						"type" => "textfield"
					...
					array(
						...
						"heading" => esc_html__("Show categories", 'soapery'),
						"description" => wp_kses_data( __("Show categories in the shortcode's header", 'soapery') ),
						"class" => "",
					...
					array(
						...
						"heading" => esc_html__("Show counters", 'soapery'),
						"description" => wp_kses_data( __("Show counters (comments and views) in the shortcode's footer", 'soapery') ),
						'dependency' => array(
							'element' => 'style',
							'value' => 'news-magazine'
						),
					...
					array(
						"param_name" => "cat",
						"heading" => esc_html__("Category", 'soapery'),
						"description" => wp_kses_data( __("Select category to show news. If empty - select news from any category or from IDs list", 'soapery') ),
						"group" => esc_html__('Query', 'soapery'),
						"class" => "",
						"value" => array_flip(soapery_array_merge(array(0 => esc_html__('- Select category -', 'soapery')), $categories)),
						"type" => "dropdown"
					),
					array(
						"param_name" => "count",
						"heading" => esc_html__("Total posts", 'soapery'),
					...
					array(
						"param_name" => "featured",
						"heading" => esc_html__("Featured posts", 'soapery'),
						"description" => wp_kses_data( __("How many posts will be displayed as featured?", 'soapery') ),
						"admin_label" => true,
						"group" => esc_html__('Query', 'soapery'),
						'dependency' => array(
							'element' => 'style',
					...
					array(
						...
						"description" => wp_kses_data( __("How many columns use to show news list", 'soapery') ),
						"group" => esc_html__('Query', 'soapery'),
						"admin_label" => true,
						'dependency' => array(
							'element' => 'style',
					...
					array(
						...
						"group" => esc_html__('Query', 'soapery'),
						"class" => "",
						"value" => array_flip(soapery_get_list_sortings()),
						"type" => "dropdown"
					),
					array(
						"param_name" => "order",
						...
						"group" => esc_html__('Query', 'soapery'),
						"class" => "",
						"value" => array_flip(soapery_get_list_orderings()),
						"type" => "dropdown"
					),
					array(
						"param_name" => "ids",
						"heading" => esc_html__("List IDs", 'soapery'),
						"description" => wp_kses_data( __("Comma separated list of post's ID. If set - parameters above (category, count, order, etc.) are ignored!", 'soapery') ),
						"group" => esc_html__('Query', 'soapery'),
						"class" => "",
						"value" => "",
						"type" => "textfield"
