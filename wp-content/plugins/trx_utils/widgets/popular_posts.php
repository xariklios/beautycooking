...
// Theme init
if (!function_exists('soapery_widget_popular_posts_theme_setup')) {
	add_action( 'soapery_action_before_init_theme', 'soapery_widget_popular_posts_theme_setup', 1 );
	function soapery_widget_popular_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('soapery_action_shortcodes_list',	'soapery_widget_popular_posts_reg_shortcodes');
		if (function_exists('soapery_exists_visual_composer') && soapery_exists_visual_composer())
			add_action('soapery_action_shortcodes_list_vc','soapery_widget_popular_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('soapery_widget_popular_posts_load')) {
	add_action( 'widgets_init', 'soapery_widget_popular_posts_load' );
	function soapery_widget_popular_posts_load() {
		register_widget('soapery_widget_popular_posts');
	}
}

// Widget Class
class soapery_widget_popular_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_popular_posts', 'description' => esc_html__('The most popular and most commented blog posts (extended)', 'soapery'));
		parent::__construct( 'soapery_widget_popular_posts', esc_html__('Soapery - Most Popular & Commented Posts', 'soapery'), $widget_ops );

		// Add thumb sizes into list
		soapery_add_thumb_sizes( array( 'layout' => 'widgets', 'w' => 75, 'h' => 75, 'title'=>esc_html__('Widgets', 'soapery') ) );
	}
	...
	function widget($args, $instance) {
		...
		$show_counters = isset($instance['show_counters']) && $instance['show_counters'] > 0 ? soapery_get_theme_option('blog_counters') : '';
		...
		for ($i=0; $i<3; $i++) {
			...
			if ($i==0) {			// Most popular
				$args['meta_key'] = 'soapery_post_views_count';
				$args['orderby'] = 'meta_value_num';
				$show_counters = $show_counters ? 'views' : '';
			} else if ($i==2) {		// Most liked
				$args['meta_key'] = 'soapery_post_likes_count';
				$args['orderby'] = 'meta_value_num';
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Widget title:', 'soapery'); ?></label>
			<input id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr($title); ?>" class="widgets_param_fullwidth" />
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_popular')); ?>"><?php esc_html_e('Most popular tab title:', 'soapery'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_commented')); ?>"><?php esc_html_e('Most commented tab title:', 'soapery'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_liked')); ?>"><?php esc_html_e('Most liked tab title:', 'soapery'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('number')); ?>"><?php esc_html_e('Number posts to show:', 'soapery'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_image')); ?>_1"><?php esc_html_e('Show post image:', 'soapery'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_image')); ?>_1"><?php esc_html_e('Show', 'soapery'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_image')); ?>_0"><?php esc_html_e('Hide', 'soapery'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_author')); ?>_1"><?php esc_html_e('Show post author:', 'soapery'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_author')); ?>_1"><?php esc_html_e('Show', 'soapery'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_author')); ?>_0"><?php esc_html_e('Hide', 'soapery'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_date')); ?>_1"><?php esc_html_e('Show post date:', 'soapery'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_date')); ?>_1"><?php esc_html_e('Show', 'soapery'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_date')); ?>_0"><?php esc_html_e('Hide', 'soapery'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('Show post counters:', 'soapery'); ?></label><br />
...
*/
if ( !function_exists( 'soapery_sc_widget_popular_posts' ) ) {
	function soapery_sc_widget_popular_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'soapery_widget_popular_posts';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_popular_posts' 
								. (soapery_exists_visual_composer() ? ' vc_widget_popular_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, soapery_prepare_widgets_args(soapery_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_popular_posts', 'widget_popular_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('soapery_shortcode_output', $output, 'trx_widget_popular_posts', $atts, $content);
	}
	soapery_require_shortcode("trx_widget_popular_posts", "soapery_sc_widget_popular_posts");
}


// Add [trx_widget_popular_posts] in the VC shortcodes list
if (!function_exists('soapery_widget_popular_posts_reg_shortcodes_vc')) {
	//add_action('soapery_action_shortcodes_list_vc','soapery_widget_popular_posts_reg_shortcodes_vc');
	function soapery_widget_popular_posts_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Popular Posts", 'soapery'),
				"description" => wp_kses_data( __("Insert popular posts list with thumbs and post's meta", 'soapery') ),
				...
				"params" => array(
					...
					array(
						...
						"heading" => esc_html__("Most popular tab title", 'soapery'),
						"description" => wp_kses_data( __("Most popular tab title", 'soapery') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("Most commented tab title", 'soapery'),
						"description" => wp_kses_data( __("Most commented tab title", 'soapery') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("Most liked tab title", 'soapery'),
						"description" => wp_kses_data( __("Most liked tab title", 'soapery') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						"param_name" => "number",
						"heading" => esc_html__("Number posts to show", 'soapery'),
						"description" => wp_kses_data( __("How many posts display in widget?", 'soapery') ),
						"admin_label" => true,
						"class" => "",
						"value" => "4",
						"type" => "textfield"
					...
					array(
						"param_name" => "cat",
						"heading" => esc_html__("Parent category", 'soapery'),
						"description" => wp_kses_data( __("Select parent category. If empty - show posts from any category", 'soapery') ),
						"class" => "",
					...
					array(
						...
						"heading" => esc_html__("Show post's image", 'soapery'),
						"description" => wp_kses_data( __("Do you want display post's featured image?", 'soapery') ),
					...
					array(
						...
						"heading" => esc_html__("Show post's author", 'soapery'),
						"description" => wp_kses_data( __("Do you want display post's author?", 'soapery') ),
						"group" => esc_html__('Details', 'soapery'),
						"class" => "",
						"std" => 1,
					...
					array(
						...
						"heading" => esc_html__("Show post's date", 'soapery'),
						"description" => wp_kses_data( __("Do you want display post's publish date?", 'soapery') ),
						"group" => esc_html__('Details', 'soapery'),
						"class" => "",
						"std" => 1,
					...
					array(
						...
						"heading" => esc_html__("Show post's counters", 'soapery'),
						"description" => wp_kses_data( __("Do you want display post's counters?", 'soapery') ),
						"group" => esc_html__('Details', 'soapery'),
						"class" => "",
						"std" => 1,
