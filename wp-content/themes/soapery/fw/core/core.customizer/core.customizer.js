...
jQuery(document).ready(function(){
	...
	jQuery('.to_demo_wrap').each(function(idx) {
		"use strict";
		var tab = jQuery(this).parents('.soapery_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		var info = jQuery(this).parents('.soapery_options_field_info').attr('id', 'to_demo_scheme_'+scheme).attr('data-scheme', scheme).data('scheme', scheme);
		if (idx==0 && jQuery(this).find('.to_demo_pin').length==1 && soapery_get_cookie('to_demo_fixed')==1) soapery_options_fix_demo(info);
	});

	// Stick demo panel
	jQuery(document).on('click', '.soapery_options_field_info .to_demo_pin', function(e) {
		soapery_options_fix_demo(jQuery(this).parents('.soapery_options_field_info'));
		e.preventDefault();
		return false;
	});	

	// Update demo on page load
	soapery_options_decorate_demo();

	// Update demo on change fields
	jQuery('#partition_schemes_content').on('change', '.soapery_options_input,.soapery_options_field input[type="hidden"]', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.soapery_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		soapery_options_decorate_demo(scheme);
	});

	// Update demo on blur color fields
	jQuery('#partition_schemes_content').on('blur', '.soapery_options_input_color', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.soapery_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
	...
	jQuery('#partition_schemes_content .soapery_options_tab > ul').sortable({
		update: function(event, ui) {
			var tabs = "";
			ui.item.parents('.soapery_options_tab').find('> ul > li').each(function() {
				tabs += (tabs ? SOAPERY_STORAGE['to_delimiter'] : '') + jQuery(this).attr('id').substr(4);
			});
			ui.item.parents('.soapery_options_tab').find('> .soapery_options_schemes_order').val(tabs);
		}
	}).disableSelection().after('<input type="hidden" name="soapery_options_schemes_order" class="soapery_options_schemes_order" value="">');
});


// When switch tabs - update fixed demo block
function soapery_init_hidden_elements(panel) {
	"use strict";
	var demo = panel.find('.soapery_options_field_info[data-scheme]');
	if (demo.length == 0) return;
	var info2 = jQuery('.soapery_options').find('>.soapery_options_field_info');
...
function soapery_options_action_scheme_delete(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.soapery_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	soapery_message_confirm(SOAPERY_STORAGE['to_strings']['scheme_delete_confirm'], SOAPERY_STORAGE['to_strings']['scheme_delete']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'soapery_options_scheme_delete',
			nonce: SOAPERY_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.soapery_options_tab').find('> .soapery_options_schemes_order').val()
		};
		setTimeout(function(){
			soapery_message_info(SOAPERY_STORAGE['to_strings']['recompile_styles'], SOAPERY_STORAGE['to_strings']['wait'], 'spin3 animate-spin', 60000);
		}, 600);
		jQuery.post(SOAPERY_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				soapery_message_success(SOAPERY_STORAGE['to_strings']['scheme_delete_complete']+'<br>'+SOAPERY_STORAGE['to_strings']['reload_page'], SOAPERY_STORAGE['to_strings']['scheme_delete']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				soapery_message_warning(SOAPERY_STORAGE['to_strings']['scheme_delete_failed']+':<br>'+rez.error, SOAPERY_STORAGE['to_strings']['scheme_delete']);
			}
		});
...
function soapery_options_action_scheme_copy(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.soapery_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	soapery_message_confirm(SOAPERY_STORAGE['to_strings']['scheme_copy_confirm'], SOAPERY_STORAGE['to_strings']['scheme_copy']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'soapery_options_scheme_copy',
			nonce: SOAPERY_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.soapery_options_tab').find('> .soapery_options_schemes_order').val()
		};
		...
		jQuery.post(SOAPERY_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				soapery_message_success(SOAPERY_STORAGE['to_strings']['scheme_copy_complete']+'<br>'+SOAPERY_STORAGE['to_strings']['reload_page'], SOAPERY_STORAGE['to_strings']['scheme_copy']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				soapery_message_warning(SOAPERY_STORAGE['to_strings']['scheme_copy_failed']+':<br>'+rez.error, SOAPERY_STORAGE['to_strings']['scheme_copy']);
			}
		});
...
function soapery_options_fix_demo(info) {
	var info2 = info.parents('.soapery_options').toggleClass('to_demo_fixed').find('>.soapery_options_field_info');
	if (info2.length == 0) {
		var clonedObj = info.clone();
		clonedObj.removeAttr('id');
		info.parents('.soapery_options').append(clonedObj);
		info2 = info.parents('.soapery_options').find('>.soapery_options_field_info');
	}
	soapery_set_cookie('to_demo_fixed', jQuery('.soapery_options').hasClass('to_demo_fixed') ? 1 : 0, 365);
}


// Decorate demo block
function soapery_options_decorate_demo() {
	"use strict";
	...
	jQuery('.to_demo_body').each(function() {
		...
		var scheme = jQuery(this).parents('.soapery_options_field_info').data('scheme');
		...
		if (jQuery(this).find('.to_demo_accent3').length > 0) {
			...
			jQuery(this).find('.to_demo_accent3_hover_bg').css({'backgroundColor': (clr=tab.find('[name="'+scheme+'-accent3_hover"]').val()) ? clr : 'transparent'});
				...
				// Accent 4
		if (jQuery(this).find('.to_demo_accent4').length > 0) {
			jQuery(this).find('.to_demo_accent4').css({'color': (clr=tab.find('[name="'+scheme+'-accent4"]').val()) ? clr : 'inherit'});
			jQuery(this).find('.to_demo_accent4_hover').css({'color': (clr=tab.find('[name="'+scheme+'-accent4_hover"]').val()) ? clr : 'inherit'});
			jQuery(this).find('.to_demo_accent4_bg').css({'backgroundColor': (clr=tab.find('[name="'+scheme+'-accent4"]').val()) ? clr : 'transparent'});
			jQuery(this).find('.to_demo_accent4_hover_bg').css({'backgroundColor': (clr=tab.find('[name="'+scheme+'-accent4_hover"]').val()) ? clr : 'transparent'});
		}
