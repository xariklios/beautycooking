<?php
$header_options = soapery_storage_get('header_mobile');
$contact_address_1 = trim(soapery_get_custom_option('contact_address_1'));
$contact_address_2 = trim(soapery_get_custom_option('contact_address_2'));
...
?>
	<div class="header_mobile">
		<div class="content_wrap">
			...
			<?php 
			soapery_show_logo(false, true, false, false, false, false); 
		...
		<div class="side_wrap">
			<div class="close"><?php esc_html_e('Close', 'soapery'); ?></div>
			<div class="panel_top">
				<nav class="menu_main_nav_area">
					<?php
						$menu_main = soapery_get_nav_menu('menu_main');
						if (empty($menu_main)) $menu_main = soapery_get_nav_menu();
						$menu_main = soapery_set_tag_attrib($menu_main, '<ul>', 'id', 'menu_mobile');
						soapery_show_layout($menu_main);
					?>
				</nav>
				<?php 
				if ($header_options['search'] && soapery_get_custom_option('show_search')=='yes')
					soapery_show_layout(soapery_sc_search(array()));
				
				if ($header_options['login']) {
					if ( is_user_logged_in() ) { 
						?>
						<div class="login"><a href="<?php echo esc_url(wp_logout_url(home_url('/'))); ?>" class="popup_link"><?php esc_html_e('Logout', 'soapery'); ?></a></div>
						<?php
					} else {
						// Load core messages
			...
			<div class="panel_middle">
				...
				if ($header_options['contact_address'] && (!empty($contact_address_1) || !empty($contact_address_2))) {
					?><div class="contact_field contact_address">
								...
								<span class="contact_label contact_address_1"><?php soapery_show_layout($contact_address_1); ?></span>
								<span class="contact_address_2"><?php soapery_show_layout($contact_address_2); ?></span>
							</div><?php
				}
			...
			<div class="panel_bottom">
				<?php if ($header_options['socials'] && soapery_get_custom_option('show_socials')=='yes') { ?>
