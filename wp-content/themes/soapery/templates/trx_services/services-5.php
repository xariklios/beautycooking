...
if ( !function_exists( 'soapery_template_services_5_theme_setup' ) ) {
	add_action( 'soapery_action_before_init_theme', 'soapery_template_services_5_theme_setup', 1 );
	function soapery_template_services_5_theme_setup() {
		soapery_add_template(array(
			'layout' => 'services-5',
			'template' => 'services-5',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 5/', 'soapery'),
...
// Template output
if ( !function_exists( 'soapery_template_services_5_output' ) ) {
	function soapery_template_services_5_output($post_options, $post_data) {
					...
					<div class="sc_services_item_description">
						...
						if ($post_data['post_protected']) {
							...
							if (!empty($post_data['post_link']) && !soapery_param_is_off($post_options['readmore'])) {
								?><a href="<?php echo esc_url($post_data['post_link']); ?>" class="sc_services_item_readmore"><?php soapery_show_layout($post_options['readmore']); ?><span class="icon-right"></span></a><?php
							}
						}
						?>
					</div>
