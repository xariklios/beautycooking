...
if ( !function_exists( 'soapery_template_attachment_theme_setup' ) ) {
	add_action( 'soapery_action_before_init_theme', 'soapery_template_attachment_theme_setup', 1 );
	function soapery_template_attachment_theme_setup() {
		soapery_add_template(array(
			'layout' => 'attachment',
			'mode'   => 'internal',
			'title'  => esc_html__('Attachment page layout', 'soapery'),
...
// Template output
if ( !function_exists( 'soapery_template_attachment_output' ) ) {
	function soapery_template_attachment_output($post_options, $post_data) {
		...
		<article <?php post_class('post_item post_item_attachment template_attachment'); ?>>
			...
			<div class="post_featured">
				<div class="post_thumb post_nav" data-image="<?php echo esc_url($post_data['post_attachment']); ?>" data-title="<?php echo esc_attr($post_data['post_title']); ?>">
					<?php
					soapery_show_layout($post_data['post_thumb']);
					$post = get_post();
					...
					if ( isset( $attachments[ $k-1 ] ) ) {
						...
						$desc = soapery_strshort(!empty($attachments[ $k-1 ]->post_excerpt) ? $attachments[ $k-1 ]->post_excerpt : $attachments[ $k-1 ]->post_title, 30);
						?>
						<a class="post_nav_item post_nav_prev" href="<?php echo esc_url($link); ?>">
							<span class="post_nav_info">
								<span class="post_nav_info_title"><?php esc_html_e('Previous item', 'soapery'); ?></span>
								<span class="post_nav_info_description"><?php soapery_show_layout($desc); ?></span>
							</span>
						</a>
						<?php
					}
					if ( isset( $attachments[ $k+1 ] ) ) {
						...
						<a class="post_nav_item post_nav_next" href="<?php echo esc_url($link); ?>">
							<span class="post_nav_info">
								<span class="post_nav_info_title"><?php esc_html_e('Next item', 'soapery'); ?></span>
								<span class="post_nav_info_description"><?php soapery_show_layout($desc); ?></span>
							</span>
						</a>
						<?php
			...
			<div class="post_content">
				...
				if ($post_data['post_protected']) { 
					...
					echo !empty($post_data['post_content']) ? trim($post_data['post_content']) : esc_html__('No image description ...', 'soapery'); 
					wp_link_pages( array( 
						'before' => '<div class="nav_pages_parts"><span class="pages">' . esc_html__( 'Pages:', 'soapery' ) . '</span>', 
						'after' => '</div>',
					...
					if ( soapery_get_custom_option('show_post_tags') == 'yes' && !empty($post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links)) {
						...
						<div class="post_info post_info_bottom">
							<span class="post_info_item post_info_tags"><?php esc_html_e('Tags:', 'soapery'); ?> <?php echo join(', ', $post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links); ?></span>
						</div>
						<?php 
					}
				}
