...
if ( !function_exists( 'soapery_template_testimonials_3_theme_setup' ) ) {
	add_action( 'soapery_action_before_init_theme', 'soapery_template_testimonials_3_theme_setup', 1 );
	function soapery_template_testimonials_3_theme_setup() {
		soapery_add_template(array(
			...
			'title'  => esc_html__('Testimonials /Style 3/', 'soapery'),
...
// Template output
if ( !function_exists( 'soapery_template_testimonials_3_output' ) ) {
	function soapery_template_testimonials_3_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?> class="sc_testimonial_item<?php echo (!empty($post_options['tag_class']) ? ' '.esc_attr($post_options['tag_class']) : ''); ?>"<?php echo !empty($post_options['tag_css']) ? ' style="'.esc_attr($post_options['tag_css']).'"' : '';?>>
				<div class="sc_testimonial_content"><?php soapery_show_layout($post_data['post_content']); ?></div>
				...
				<div class="sc_testimonial_avatar"><?php soapery_show_layout($post_options['photo']); ?></div>
				<?php } ?>
			</div>
		<?php
